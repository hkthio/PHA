...
/* Version 1.3: added possibility if only top trace is given, contours will be created based on dip */
/* Version 1.5: corrected gridding in the case of complex contours */
...
int main()
   ...
   double azk[MAX_CONT],az,baz,gcarc,rot[3][3],irot[3][3];
   double pi;

   /* contour variables */
   struct geographic co[MAX_CONT][500],cr[MAX_CONT][500];
   ...
   scanf("%f",&conv_az);
   printf("Grid length and width?\n");
   scanf("%f %f",&dL, &dW);
   dL=dL/111.;
   printf("Grid bottom depth?\n");
   scanf("%f",&dmax);
   printf("Initial dip?\n");
   scanf("%f",&daz);
   ...
   while(fscanf(fp,"%s %d%*[^\n]\n",cdum,(np+k)) == 2)
      ...
      if(k >= MAX_CONT)
         {
         printf("Too many contours, increase MAX_CONT\n");
         return -1;
         }
      for(i=0;i<np[k];i++) 
         {
         fscanf(fp,"%lf %lf %lf",&co[k][i].lon,&co[k][i].lat,&co[k][i].dep);
         if(co[k][i].dep < 0.0) co[k][i].dep=-co[k][i].dep;
         }
      k++;
      }
   fclose(fp);
   ...
   if(ncont == 1)
      ...
      if(np[0]==2)
         {
         co[0][3]=co[0][1];
         distaz(co[0][0],co[0][3],&gcarc,&az,&baz);
         hdist=gcarc/3;
         for(i=1;i<3;i++)
            {
            distaz(co[0][i-1],co[0][3],&gcarc,&az,&baz);
            co[0][i]=newloc(co[0][i-1],az,hdist);
            }
         np[0]=4;
         }
      fp=fopen("o_extra","w");
   ...
   for(i=0;i<nx;i++)
      ...
      for(k=1;k<ncont;k++)
         {
         distaz(sc[k-1][i],sc[k][i],&gcarc,(azk+k),&baz);
         x[k]=x[k-1]+gcarc*111.;
         y[k]=sc[k][i].dep;
         printf("%d\n",k);
         }
      ...
      while(yprev< dmax)
         ...
         yprev=yy;
         k=0;
         while(k< ncont && xx > x[k]) k++;
         /*printf("k=%d\n",k);*/
         sd[j][i]=newloc(sc[k-1][i],azk[k],(xx-x[k-1])/111.);
         sd[j][i].dep=yy;
